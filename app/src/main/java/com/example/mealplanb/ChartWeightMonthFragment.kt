...
class ChartWeightMonthFragment : Fragment(), StatView, StatWeightView {
...
//        val monthList = getMonthDateList()
        ...
        authService.statWeightMonthCheck()
...
//        }
    ...
    override fun StatWeightMonthCheckSuccess(statistic_type: String, weights: List<MonthlyWeight>) {
        ...
        for(i in weights.indices) { //indices : dataList의 최소 index ~ 최대 index
            chartEntry.add(Entry(i.toFloat(),weights[i].monthAverageWeight.toFloat()))
            dataList.add(ChartCommitData(getDate(weights[i].month),weights[i].monthAverageWeight.toInt()))
        }

        if(dataList.size == 1) {
            chartEntry.add(Entry(1.toFloat(),weights[0].monthAverageWeight.toFloat()))
            dataList.add(ChartCommitData(getDate(weights[0].month),weights[0].monthAverageWeight.toInt()))
        }
